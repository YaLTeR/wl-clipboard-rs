...
use failure::Fail;
use wayland_client::{
    global_filter, protocol::wl_seat::WlSeat, ConnectError, Display, EventQueue, GlobalManager, Interface, NewProxy,
};
...
pub fn initialize(primary: bool, socket_name: Option<OsString>) -> Result<CommonData, Error> {
    ...
    let clipboard_manager = global_manager.instantiate_exact::<ZwlrDataControlManagerV1, _>(data_control_version,
                                                                                            impl_manager)
                                          .map_err(|_| Error::MissingProtocol { name: ZwlrDataControlManagerV1::NAME,
